import { threadsApi, votesApi } from '@/useAPI';
...
export const threadsAction = {
  create(thread) {
    return { type: THREADS_ACTION_TYPE.CREATE, payload: { thread } };
  },
  set(threads) {
    return { type: THREADS_ACTION_TYPE.SET, payload: { threads } };
  },
  unset() {
    return { type: THREADS_ACTION_TYPE.UNSET, payload: {} };
  },
  upvote({ threadId, userId }) {
    return { type: THREADS_ACTION_TYPE.UP_VOTE, payload: { threadId, userId } };
  },
  downvote({ threadId, userId }) {
    return { type: THREADS_ACTION_TYPE.DOWN_VOTE, payload: { threadId, userId } };
  },
  neutralizeUpVote({ threadId, userId }) {
    return { type: THREADS_ACTION_TYPE.NEUTRALIZE_UP_VOTE, payload: { threadId, userId } };
  },
  neutralizeDownVote({ threadId, userId }) {
    return { type: THREADS_ACTION_TYPE.NEUTRALIZE_DOWN_VOTE, payload: { threadId, userId } };
  },
};
...
export const threadsThunks = {
  asyncCreate({ title, body, category }) {
    return async (dispatch) => {
      ...
      const threadsData = await threadsApi.createThread({ title, body, category });
  ...
  asyncUpvote(threadId) {
    return async (dispatch, getState) => {
      ...
      dispatch(threadsAction.upvote({ threadId, userId: authUser.id }));
      const threadsData = await votesApi.upvoteThread(threadId);
      if (threadsData.status === 'fail') {
        dispatch(threadsAction.neutralizeUpVote({ threadId, userId: authUser.id }));
        dispatch(hideLoading());
        throw new Error(threadsData.message);
      }
      dispatch(hideLoading());
  ...
  asyncDownvote(threadId) {
    return async (dispatch, getState) => {
      dispatch(showLoading());
      const { authUser } = getState();
      dispatch(threadsAction.downvote({ threadId, userId: authUser.id }));
      const threadsData = await votesApi.downvoteThread(threadId);
      if (threadsData.status === 'fail') {
        dispatch(threadsAction.neutralizeDownVote({ threadId, userId: authUser.id }));
        dispatch(hideLoading());
        throw new Error(threadsData.message);
      }
      dispatch(hideLoading());
